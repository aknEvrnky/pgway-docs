...
pgway pushes **OTLP/gRPC metrics** — there is no Prometheus `/metrics` scrape endpoint. To view the metrics locally without assembling your own pipeline, the repo ships a bundled Docker Compose stack under `docker/observability/`:

| Component | Purpose | Image |
|-----------|---------|-------|
| OTel Collector | receives OTLP (gRPC `:4317`, HTTP `:4318`), re-exports as Prometheus | `otel/opentelemetry-collector:0.161.0` |
